...
//         static String globalLastRecordUpdateTime;
...
//         {
//             if (globalLastRecordUpdateTime == null)
//             {
//                 globalLastRecordUpdateTime = String.Empty;
//             }
...
//             string localLastRecordUpdateTime = String.Empty;
...
//             {
//                 conn.Open();
//                 var text = "SELECT id,KennelName,KennelFacebookId,KennelFacebookToken,KennelShortName,Latitude,Longitude,KennelFacebookImportDaysInPast,KennelFacebookImportDaysInFuture,KennelFacebookForceUpdatesUntil FROM HC.Kennel WHERE KennelFacebookToken IS NOT NULL;";
...
//                         DateTime forceUpdatesUntil = DateTime.MinValue;

//                         try
//                         {
//                             if (rows.GetValue(9) != null)
//                             {
//                                 forceUpdatesUntil = DateTime.Parse(rows.GetValue(9).ToString().Trim());
//                             }                  
//                         }
...
//                                 var request = String.Format("{0}?fields=events.limit(1000).since({1}).until({2}).fields(updated_time,cover,name,place,description,start_time,type,category,id)&access_token={3}", KennelFacebookId, DateTime.Now.AddDays(-(daysInPast + 2)).ToString("MM/dd/yy"), DateTime.Now.AddDays(daysInFuture + 2).ToString("MM/dd/yy"), KennelFacebookToken);
...
//                                             string UpdatedTime = jsonRes.Events.Data[i].UpdatedTime.Substring(0, 22) + ":" + jsonRes.Events.Data[i].UpdatedTime.Substring(22, 2);
//                                             log.LogInformation("Updated Time = " + UpdatedTime + " for event: " + Name);

//                                             // only process updates the first time the Azure function is run (globalLastRecordUpdateTime is empty)
//                                             // or if the UpdatedTime of the record is after the globalLastRecordUpdateTime
//                                             // or the forceUpdatesUntil is not null and is in the future
//                                             if ((globalLastRecordUpdateTime.Length == 0) || (String.Compare(globalLastRecordUpdateTime,UpdatedTime) < 0) || (forceUpdatesUntil > DateTime.Now))
//                                             {
//                                                 if ((localLastRecordUpdateTime.Length == 0) || (String.Compare(localLastRecordUpdateTime, UpdatedTime) < 0))
//                                                 {
//                                                     localLastRecordUpdateTime = UpdatedTime;
//                                                 }
...
//                                                                         await mmClient.SendNotification("Run updated", updateRows.GetValue(0).ToString(), updateRows.GetValue(1).ToString(), log);
...
//             if (localLastRecordUpdateTime.Length > 0)
//             {
//                 globalLastRecordUpdateTime = localLastRecordUpdateTime;
//             }
