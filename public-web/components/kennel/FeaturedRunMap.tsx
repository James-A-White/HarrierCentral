...
export default function FeaturedRunMap({ run }: { run: RunEvent }) {
  if (!run.Latitude || !run.Longitude) {
    return (
      <div className="h-full flex flex-col items-center justify-center gap-3" style={{ color: "var(--kennel-text-muted)" }}>
    ...
    <div className="relative h-full w-full">
      ...
      <div className="absolute bottom-0 left-0 right-0 z-[1000] bg-black/70 backdrop-blur-sm px-4 py-3 space-y-0.5">
        {run.LocationOneLineDesc && (
          <p className="text-sm font-semibold text-white leading-snug">{run.LocationOneLineDesc}</p>
        )}
        {addressLines.map((line, i) => (
          <p key={i} className="text-xs text-white/70 leading-snug">{line}</p>
        ))}
        {w3wUrl && (
          <a
            href={w3wUrl}
