...
export function checkpointIcon(m: MarkEntry, ICON_PX: number = DEFAULT_ICON_PX): L.DivIcon {
  const isTile = !!(m.glyphUrl || m.text);
  const key = `${ICON_PX}|${m.iconUrl ?? ""}|${m.glyphUrl ?? ""}|${m.glyphFixed}|${m.text ?? ""}|${m.label ?? ""}|${m.isCaution}`;
