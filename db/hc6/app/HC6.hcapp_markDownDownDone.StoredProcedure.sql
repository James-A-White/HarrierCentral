CREATE OR ALTER PROCEDURE [HC6].[hcapp_markDownDownDone]
...
AS
-- =====================================================================
-- Procedure: HC6.hcapp_markDownDownDone
-- Description: Marks a DownDown charge as done (IsDone = 1). The
--   operation is idempotent — calling it on an already-done charge
--   succeeds without error. Auth: GM (0x02) or RA (0x08).
-- Parameters:
--   @deviceId    - Registered device UUID
--   @accessToken - Token validated against DeviceSecret
--   @kennelId    - Kennel that owns the event (auth scope)
--   @eventId     - Event the DownDown belongs to (validation scope)
--   @downDownId  - DownDown to mark as done
-- Returns:
...
BEGIN
    ...
    SELECT @errorId AS errorId, @errorType AS errorType, @errorCode AS errorCode,
           ...
           'Only the GM and RAs can mark DownDowns as done.' AS errorUserMessage,
           @procName AS errorProc;
    RETURN;
END
