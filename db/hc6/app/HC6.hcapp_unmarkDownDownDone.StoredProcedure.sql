CREATE OR ALTER PROCEDURE [HC6].[hcapp_unmarkDownDownDone]
...
AS
-- =====================================================================
-- Procedure: HC6.hcapp_unmarkDownDownDone
-- Description: Clears the done flag on a DownDown charge (IsDone = 0).
--   The operation is idempotent — calling it on an already-pending
--   charge succeeds without error. Auth: GM (0x02) or RA (0x08).
-- Parameters:
--   @deviceId    - Registered device UUID
--   @accessToken - Token validated against DeviceSecret
--   @kennelId    - Kennel that owns the event (auth scope)
--   @eventId     - Event the DownDown belongs to (validation scope)
--   @downDownId  - DownDown to unmark
-- Returns:
