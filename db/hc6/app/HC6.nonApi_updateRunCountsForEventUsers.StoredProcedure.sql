-- =====================================================================
-- Procedure: HC6.nonApi_updateRunCountsForEventUsers
-- Description: Recalculates run-count statistics for every non-anonymous
--              hasher who has a HasherEventMap row for @eventId. Applies
--              the identical 5-stage logic as
--              nonApi_updateRunCountsForAllUsers, but #affected is scoped
--              to this event's attendees instead of a datetime watermark:
--                1. Inserts HasherKennelMap rows for new kennel
--                   relationships discovered for those users.
--                2. Updates run-count columns on HasherEventMap.
--                3. Clears counts on non-qualifying HEM rows.
--                4. Updates HasherKennelMap summary counters.
--                5. Updates rolling-year counts on HasherKennelMap.
--
--              Called by addEditEvent AFTER nonApi_updateRunNumbers so an
--              event edit refreshes its attendees' counts immediately,
--              rather than leaving them stale until the nightly full sweep.
--              (nonApi_updateRunNumbers only updates HC.Event, so attendee
--              HEM.updatedAt is not bumped and a watermark-based recompute
--              would miss them.)
-- Parameters:
--   @eventId - The event whose attendees' run counts to recompute.
-- Returns:    Nothing. Internal helper. No TRY/CATCH by design — it runs
--             inside the caller's transaction, so any error propagates to
--             the caller's CATCH (matches nonApi_updateRunCountsForAllUsers).
--
-- KNOWN LIMITATION (accepted edge case — self-heals nightly):
--   When an event edit triggers a run-number CASCADE — i.e.
--   nonApi_updateRunNumbers renumbers this event AND later events in the
--   kennel — the attendees of those OTHER events also have shifted counts
--   but are NOT in this event's #affected, so they are not corrected here.
--   Those users' counts stay briefly stale until the nightly HC6 full
--   recompute (MemberStandingSweep, 03:10 UTC) cleans them up. This is a
--   deliberate trade-off to keep the edit-time recompute event-scoped and
--   cheap. If that tail ever proves material, widen the scope to the set of
--   events whose EventNumber actually changed.
-- Author: Harrier Central
-- Created: 2026-07-25
-- HC5 Source: HC.nonApi_updateRunCountsForEventUsers (logic modernised to
--   match HC6.nonApi_updateRunCountsForAllUsers — EventStartLocal/Gmt
--   columns, inlined counts, idempotent guards).
...
-- =====================================================================
CREATE OR ALTER PROCEDURE [HC6].[nonApi_updateRunCountsForEventUsers]
...
BEGIN
    ...
    -- Materialise the set of affected non-anonymous users once: everyone who
    -- has a HasherEventMap row for this event. All 5 stages JOIN to this table.
    ...
    VALUES ('RUN COUNTS', 'Updated run counts for event users');
