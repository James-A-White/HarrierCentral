...
DECLARE @newestSeq INT =
    (SELECT MAX(em.MessageSequenceCount) FROM HC.EventMessage em
      WHERE em.EventId IS NULL AND em.KennelId IS NULL
        AND em.MessageType = 1 AND em.Removed = 0);

DECLARE @lastRead INT =
    ISNULL((SELECT b.LastSequenceCount FROM HC.EventMessageBadgeCounts b
             WHERE b.UserId = @userId AND b.EventId IS NULL AND b.KennelId IS NULL), 0);
...
BEGIN
    -- Same IS NULL join as the send SP. Comparing a nullable column to a
    -- NULL value never matches, so the kennel form of this MERGE would add
    -- a new badge row on every read.
    ...
    WHEN NOT MATCHED BY TARGET THEN
        INSERT (UserId, EventId, KennelId, LastSequenceCount, LastReadAt)
        VALUES (Source.UserId, NULL, NULL, Source.LastSequenceCount, SYSDATETIMEOFFSET());
END
