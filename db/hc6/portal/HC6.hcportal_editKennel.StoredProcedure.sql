CREATE OR ALTER PROCEDURE [HC6].[hcportal_editKennel]
	...
	@defaultRunDayOfWeek SMALLINT = NULL,
...
--             @trailTypesConfigJson (nullable) — per-kennel PackTrack trail-type
--               config JSON; NULL = leave unchanged (clients use defaults)
...
BEGIN TRY
	...
	SET
		...
		DefaultRunDayOfWeek = COALESCE(@defaultRunDayOfWeek, DefaultRunDayOfWeek),
		...
		AllowCredit = COALESCE(@allowCredit, AllowCredit),
