...
CREATE PROCEDURE [HC5].[hcapp_getEventBadgeCount]
...
BEGIN
	...
	IF (@resetAllBadgeCounts != 0)
		BEGIN
			...
			UPDATE embc SET embc.LastSequenceCount = (SELECT max(em.MessageSequenceCount) FROM HC.EventMessage em where em.EventId = embc.EventId)
		...
		BEGIN
			IF @publicEventId IS NOT NULL
				BEGIN
					...
					SELECT coalesce(@messageSequenceCount,0) - embc.LastSequenceCount as BadgeCount, @publicEventId as PublicEventId
					FROM HC.EventMessageBadgeCounts embc 
					WHERE embc.EventId = @eventId AND embc.UserId = @userId
				END
			ELSE
				BEGIN
					SELECT max(em.MessageSequenceCount) - embc.LastSequenceCount as BadgeCount,em.PublicEventId
					FROM HC.EventMessageBadgeCounts embc 
