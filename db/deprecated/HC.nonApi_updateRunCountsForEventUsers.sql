CREATE OR ALTER PROCEDURE [HC].[nonApi_updateRunCountsForEventUsers]
...
-- =====================================================================
    ...
    VALUES ('OBSOLETE SP', 'Obsolete SP called: HC.nonApi_updateRunCountsForEventUsers (ignored; HC6 owns run counts)');
    RETURN;
...
BEGIN
	...
	from HC.HasherEventMap hem
	inner join HC5.fn_GetRunCountsByEvent(@eventId) rc  on rc.hemId = hem.id
	...
	from HC.HasherKennelMap hkm 
		...
		LEFT OUTER JOIN HC.HasherEventMap hem on 
			hem.KennelId = hkm.KennelId 
			and hem.UserId = hkm.UserId 
			and hem.AttendenceState >= 20
		INNER JOIN HC.Event evt on hem.EventId = evt.id 
			AND (cast(evt.EventStartDatetime as datetime) AT TIME ZONE tz.Timezone) AT TIME ZONE 'UTC' < getdate()
...
update hkm set
	...
	FROM HC.HasherKennelMap hkm 
	INNER JOIN HC.Hasher h on hkm.UserId = h.id
	INNER JOIN HC5.fn_GetRunCountsRollingByEvent(@eventId) rc on hkm.UserId = rc.userId and hkm.KennelId = rc.KennelId
	WHERE 
		h.isAnonymous = 0
		AND
		(
