...
class RegionsModel {
  RegionsModel({this.regionId, this.regionName, this.countryId, this.flagFile, this.removed, this.updatedAt});
...
class RegionsTableHelper {
  ...
  static const IntPrefsEnum lastCacheClearKey = IntPrefsEnum.lastCacheClearRegionsData;
  ...
  static final RegionsTableHelper instance = RegionsTableHelper._privateConstructor();
  ...
  static Map<String, dynamic> toMap(RegionsModel item) {
    final Map<String, dynamic> map = <String, dynamic>{
      ...
      RegionsTableHelper.colUpdatedAtValue: item.updatedAt.millisecondsSinceEpoch,
...
class RegionsService {
  ...
  Future<void> clearTable() async {
    ...
    await db.rawDelete('DELETE FROM ${RegionsTableHelper.tableName}').then((void dummy) {
      setIntPref(RegionsTableHelper.lastCacheClearKey, DateTime.now().millisecondsSinceEpoch);
    });
  }
  ...
  Future<void> updateDatabase(List<RegionsModel> items) async {
    ...
    for (int i = 0; i < items?.length ?? 0; i++) {
      ...
      final List<Map<String, dynamic>> table = await db.rawQuery('SELECT * FROM ${RegionsTableHelper.tableName} WHERE ${RegionsTableHelper.remoteDbId} = "${items[i].regionId}"');
      if ((table == null) || (table.isEmpty)) {
        await db.transaction<dynamic>((Transaction txn) async {
          final int result = await txn.insert(RegionsTableHelper.tableName, row);
          print(result.toString() + ' inserted into to the ${RegionsTableHelper.tableName} table @ ${DateTime.now().millisecondsSinceEpoch.toString()}');
        });
      } else {
        final String rowId = table.first['id'].toString();

        await db.transaction<dynamic>((Transaction txn) async {
          final int result = await txn.update(RegionsTableHelper.tableName, row, where: 'id = $rowId');
          print(result.toString() + ' update to the ${RegionsTableHelper.tableName} table @ ${DateTime.now().millisecondsSinceEpoch.toString()}');
        });
      }
    }
  }
    ...
    for (int i = 0; i < jsonResultSets.length; i++) {
      ...
      for (int j = 0; j < jsonResults.length; j++) {
        ...
        if (doNormalizeMap == null) {
          ...
          if (doNormalizeMap)
          {
            print('Normalize map called for ${RegionsTableHelper.tableName}, # of fields on the wire = ${jsonItem.length}, # of fields in internal DB = ${testMap.length - 1}' );
          }
        }
        ...
        final String query = 'SELECT * FROM ${RegionsTableHelper.tableName} WHERE ${RegionsTableHelper.remoteDbId} = "${jsonItem['regionId']}"';
