...
| Track import (E5.F5.S6/S7) | SERVER-SIDE since 2026-09-11: app `lib/services/import/track_import_service.dart` (SAS from `GetTrackImportUploadToken`, 4 MB block upload to blob container `track-imports`, `hcapp_submitTrackImport`, then `ProcessTrackImport` in slices; held items resolved with `resolve:{index,eventId,replace}`) + `lib/pages/menu_pages/import_gpx_page.dart`. API `TrackImportProcessor.cs` (GPX/TCX XML, own FIT decoder, gzip, zip; thin 5 m/15 s; match via `nonApi_findRunForTrack`; write via `PositionWriter`; attendance via `nonApi_ensureTrackAttendance`), `TrackImportFunctions.cs` (token, process, 03:45 UTC backstop), job row `HC.TrackImport` (ResultJson records every activity). Rules: archive never overwrites; no start location ⇒ skip; several runs ⇒ held; single file ⇒ held for confirm/replace. `PositionWriter` is the ONE write path (StorePositions uses it too); Track* / EventTrack first/last are CAPTURE times. Files handed to the app: `lib/services/incoming_file_service.dart` ↔ `ios/Runner/IncomingFileBridge.swift` / `android MainActivity` over `harrier_central/incoming_file`; iOS share sheet = `ios/ShareExtension/` target (app group `group.com.harriercentral.app`, bundle `com.harriercentral.app.share`, created with the `xcodeproj` gem) |
