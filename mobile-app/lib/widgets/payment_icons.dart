...
class PaymentIcons extends StatelessWidget {
  ...
  Widget? _getPaymentIcon(
    ...
    switch (paymentProvider.toLowerCase()) {
      ...
      case 'transferwise':
        w = Image.asset(
          'images/logos/transferwise_logo.png',
          height: imgSize,
          width: imgSize,
        );
        break;
    ...
    return (w == null)
        ...
        : GestureDetector(
            onTap: () async {
              ...
              EnumPayForExtras didPayForExtras = payForRunOnly;
              ...
              if (extrasPrice > 0) {
                ...
                final dynamic x = await showExtrasDialog(
                  context,
                  eventPrice,
                  extrasPrice,
                );
              ...
              if (result ?? false) {
                ...
                if ((kennel.allowSelfPayment &
                        selfPaymentAutoPayAfterBankTransfer) ==
                    selfPaymentAutoPayAfterBankTransfer) {
                  // show the alert so the user knows how much to pay
                  ...
                  if (result2 ?? false) {
                    stateSetter(
                      -1,
                      -1,
                    ); // call setState on the parent to indicate we are updating
                    final List<dynamic> adHocItems = await payForEvent(
                      eventPrice + extrasPrice,
                      didPayForExtras,
                      surcharge,
                      paymentProvider,
                    );
  ...
  bool showPaymentIcons() {
    ...
    if ((DateTime.now().isBefore(
          event.eventStartDatetime.subtract(
            Duration(days: daysToDisplayPaymentIcons),
          ),
        )) ||
    ...
    if ((distToEvent == null) ||
        (distToEvent! > distanceToDisplayPaymentIcons * 1000)) {
      return false;
    }
