...
class FutureRunsListPage extends StatelessWidget {
  ...
  Widget _buildListView(
    ...
    final bool showLoadingNotEmpty =
        noRunsAvaiable &&
                    ...
                    children: [
                      showLoadingNotEmpty
                          ...
                          : Obx(() {
                              ...
                              // filteredRuns, both RxList, and the enclosing widget is a
                              // GetBuilder — which does NOT subscribe to them. So the list's
                              // itemCount only refreshed when something happened to call
                              // update(['runList']), which is not the same moment the data
                              // changed. Reading here ties the two together.
                              final List<dynamic> items =
                                  listController.displayRuns;
                              final int anchor =
                                  listController.initialScrollIndex;
                              return ScrollablePositionedList.builder(
                                // Keyed on the anchor. initialScrollIndex is only honoured
                                // when the widget is CREATED, so without a key a changed
                                // anchor is silently ignored AND the render object is reused
                                // with children laid out for the old structure. That is the
                                // shape of the paint-time crash seen 61 times on 3.0.12:
                                //   RenderSliverMultiBoxAdaptor.childMainAxisPosition
                                // NOT reproduced in a harness — this is a fix by reasoning
                                // plus the recorded footgun, not by repro (James, 2026-09-13).
                                key: ValueKey<int>(anchor),
                                ...
                                itemBuilder: (BuildContext context, int index) {
                                  ...
                                  if (items[index] is int) {
                                    ...
                                    final run =
                                        items[index] as RunDetailsAggregate;
                                    return RunListItem(
                                      key: ValueKey(
                                      ...
                                      isPastRun:
                                          listController.showsInlinePast &&
                                          index <
                                              listController.pastRuns.length,
                                      onItemTapped: () async {
...
class EventChatSummary {
  ...
  /// When the thread last had a message. What the chat list sorts on — a
  ...
  /// 2026-09-13). Everything else about the thread is unchanged.
  EventChatSummary withBadgeCount(int newBadgeCount) => EventChatSummary(
    publicEventId: publicEventId,
    badgeCount: newBadgeCount,
    eventId: eventId,
    eventName: eventName,
    eventNumber: eventNumber,
    eventStartDatetimeGmt: eventStartDatetimeGmt,
