...
namespace HcWebApi.Endpoints
{
    ...
    {
        private readonly ILogger<PortalApi> log;

        private TableClient _tableClient;

        public PortalApi(ILogger<PortalApi> logger)
        {
            log = logger;
     ...
     ?? throw new InvalidOperationException("Failed to deserialize the request body.");
            ...
            if (data.publicHasherId == null || data.accessToken == null)
            {
                log.LogInformation("Missing required parameters: publicHasherId or accessToken.");
                return new BadRequestObjectResult("Missing required parameters: publicHasherId or accessToken.");
            }
            ...
            {
                log.LogError("PortalApi SP error: {Message}", ex.Message);
        ...
        public async Task SendNotifications(List<List<Dictionary<string, object>>> multipleResults, ILogger logger)
        {
            try
            {
                ...
                // Extract the recipients (third array)
...
// Models for EventMessage and Recipient
