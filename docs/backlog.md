...
| `E3.F4.S6` | As a **Hasher**, I want a button on the run map that routes me to the start in my map app — **Get me there** on the day of the run, **Get Directions** on any day before it — so that I do not have to copy the address into another app. No button the day after or later, none once the run has PackTrack data (the map is a trail by then; synced `TrackRunnerCount`, or tracks the map has loaded, since the count lags a live run), and none when the run has no coordinates. Uses the saved map app, or the chooser. `runDirectionsFor` in `run_recency.dart` (unit-tested), `RunTrackerMap.bottomOverlay`, `Utilities.showOnMap(directions:)`. **Built 2026-09-26 on dev** — ⚠ Known gap: not yet seen on a device. | `Building` |
