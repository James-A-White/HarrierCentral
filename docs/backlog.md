...
| `E7.F1.S6` | As a **Hasher**, I want to enter a charge whether or not I am checked in so that a nomination from the circle is never refused. `hcapp_addDownDown` 1.1.0 drops the attendance check; where the kennel (or the run) lets hashers set their own attendance, entering a charge checks the caller in At Hash, otherwise their attendance is left as it was (James, 2026-09-12). Eight refusals at one German Nash Hash run on 2026-09-05 were RSVP'd members who had not checked in. SP on dev, not deployed. | `Building` |
