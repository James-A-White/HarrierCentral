...
| `E14.G3.R5` | Every harvested session log carries the device's memory, network and battery figures over time, so a resource problem is measurable per device without a repro. | `[METRICS]` lines from `DeviceMetricsService`: start, every background/foreground edge, every 15 min; process CPU time and share, RSS/PSS/headroom, app-layer bytes and latency plus (Android) process-level bytes, location-stream minutes per cost tier, database/documents/cache footprint and free disk, battery level and unplugged drain, Low Power Mode and thermal state. Native snapshot channel on both platforms. **Building** — iOS gives no per-app network or battery attribution; the line records the device's condition beside the app's own footprint instead. |
