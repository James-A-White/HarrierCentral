...
| `E9.F7.S6` | As a **returning web member**, I want signing in on a new browser to be one tap, so that the email code is a once-per-device cost, not a per-visit one. Passkeys (WebAuthn) bound to the same `HC.Device` row; Sign in with Apple/Google through the existing `hcapp_processThirdPartyLogin`. Deliberately after S1–S4: the year-long cookie gets most of the value first. | `Next` |
