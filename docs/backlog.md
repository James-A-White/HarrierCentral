...
| `E14.G3.R5` | Every harvested session log carries the device's memory, network and battery figures over time, so a resource problem is measurable per device without a repro. | `[METRICS]` lines from `DeviceMetricsService`: start, every background/foreground edge, every 15 min; RSS/PSS/headroom, app-layer and (Android) process-level bytes, battery level and unplugged drain, Low Power Mode and thermal state. Native snapshot channel on both platforms. **Building** — iOS gives no per-app network or battery attribution; the line records the device's condition beside the app's own footprint instead. |
